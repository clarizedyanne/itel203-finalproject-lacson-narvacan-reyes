<?php
// order_confirmation.php - Advance Order Success
session_start();
require_once 'includes/Auth.php';
Auth::requireLogin();

// Must arrive from order placement
...
if ($userRow) {
    sendOrderEmail(
        ...
        ['order_id' => $orderId, 'total' => $total, 'res_date' => $resDate,
         'res_time' => $resTime, 'pay_method' => $payMethod]
    );
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>Order Confirmed – Oli's SelfieTea & Coffee</title>
  ...
  <style>
    ...
    .detail-value.price { color: var(--brown); font-size: 1.05rem; }
    .action-btn {
      display: block;
      width: 100%;
  ...
  <div class="confirm-card">
    <div class="confirm-header">
      ...
      <p style="opacity:0.8; font-size:0.9rem; margin:0;">
        Your advance order has been received.
      </p>
    </div>

    <div class="confirm-body">
      <p style="font-size:0.88rem; color:var(--text-muted); margin-bottom:1.2rem; text-align:center;">
        Hi <strong><?= $userName ?></strong>! Here's your order summary:
      </p>

      <div>
        <div class="detail-row">
          <span class="detail-label"><i class="bi bi-hash me-1"></i>Order #</span>
          <span class="detail-value"><?= str_pad($orderId, 5, '0', STR_PAD_LEFT) ?></span>
        </div>
        <div class="detail-row">
        ...
        <div class="detail-row">
          <span class="detail-label"><i class="bi bi-cash-coin me-1"></i>Total Amount</span>
          <span class="detail-value price">₱<?= number_format($total, 2) ?></span>
        </div>
      </div>
      ...
      <div style="background:#f0fdf4; border-radius:12px; padding:14px 16px; margin: 1.2rem 0; font-size:0.83rem; color:var(--green-dark); line-height:1.7;">
        ...
        Thank you for choosing Oli's! Admin will review and confirm your reservation and order. 
        Your food will be ready when you arrive! Please do arrive 15 minutes early ahead of your reserved time.
      </div>
